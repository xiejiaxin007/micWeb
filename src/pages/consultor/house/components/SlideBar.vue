<!--
    @name: 文章详情固定分享按钮、一级二级菜单选中样式调整
    @description: 分享资料弹窗底部新增分享按钮
    @author: zhangchuang
    @date: 2019-08-29
-->
<template>
    <transition name="fadeInleft">
        ...
        >
                ...
                <div class="jl-menus">
                    <ul class="slider-items"
                        ...
                        <div class="slider-items-box">
                            <li
                                :class="{'active first-menu-active': currenChosen.classIndex === index}"
                    ...
                    <div class="slide-menu">
                        <div class="slide-menu-box">
                            <div
                                ...
                                @click="selectMenu('subIndex', subIndex, subDist)"
                            >
            ...
            >
                                        ...
                                        <div class="card-info">
                                            ...
                                            <div class="card-desc">
                                                ...
                                                <div class="price">
                                                    ...
                                                    <span class="size-s">{{project.acreage_min}}-{{project.acreage_max}}m²</span>
                                                </div>
                                                <div class="address">
                                                    <span class="size-s">
                                                        {{project.address}}
                ...
                <!-- 文章详情固定分享按钮 add by zhangchuang satrt-->
                <div class="jl-slider-footer">
                    <el-button
                        @click="shareModal(currentArticle)"
                        class="share-btn"
                        type="primary"
                ...
                <!-- 文章详情固定分享按钮 add by zhangchuang end-->
            </div>
            <!-- 微信分享弹窗 -->
            <el-dialog
                title="选择分享方式"
            ...
            >
                <div class="dialog-body">
                    ...
                    >
                        ...
                        <div class="msg-warn">
                            <i class="iconfont icongantanhao-yuankuang" 
                                aria-hidden="true"></i>
...
export default {
    ...
    mounted() {
        ...
        // 初始化赋值
        // city 和cityName 均为字符串，多个是以“,”拼接的
        this.cityId = this.cityInfo.currentCityId;
        if (this.show) {
            this.getData(this.cityId);
        }
        PubSub.subscribe('openArticle', (msg, data) => {
            // 打开文章列表
            this.show = true;
            this.mapChosen = data;
        });
        // 关闭文章列表
        PubSub.subscribe('closeArticle', (msg, data) => {
            this.show = false;
            if (this.player) {
                this.player.dispose();
                this.player = null;
    ...
    computed: {
        ...
        getWechatFriends() {
            return this.hasbinded.length === 0
                ? '该订单暂未绑定微信'
                : `该订单绑定的微信名称为：${this.hasbinded.join()}`;
        },
        isShowBindBtn() {
    ...
    methods: {
        ...
        async affirmHouse(val) {
            try {
                ...
                if (data.code === 0) {
                    let isFind = this.projectInfosTwo.findIndex(
                        (item, index) => {
        ...
        editProject(type, index) {
            if (type === 'delete') {
                ...
                // 超出限制校验
                if (this.projectInfos[index].isEdit && this.projectInfos[index].project_desc.length > 200) {
                                ...
                                if (item.cate_id != '20') {
                                    ...
                                    itemArr.map((subItem, index) => {
                                        if (disId.toString() === subItem.district_id) {
                                            this.currenChosen = {
                                                classIndex: parseInt(index) + areaIndex,
                                                subIndex: 0
                                            };
                                            // 滚动到可是区域
                                            this.$nextTick(() => {
                                                let dom = document.getElementsByClassName('first-menu-active');
                                                if (dom.length > 0) {
                                                    let scroll = dom[0].offsetTop;
                                                    this.$refs.silderItem.scrollTop = scroll;
                                                }
                                            });
                                        }
                                        obj = {
        ...
        // 初始化video
        async initVideo() {
            if (!this.player && this.playerOptions.sources[0].src && this.$refs.videoPlayer) {
        ...
        // 点击列表
        selectMenu(paraName, paraIndex, content) {
            // 切换前存储start
            let curr =
                this.articleList.length > 0 && this.articleList[this.currenChosen.classIndex].list[this.currenChosen.subIndex]
        ...
        async bindWechat() {
            try {
                ...
                // 第三个参数为固定参数7
                const { data: { msg } } = await addWxContact(this.orderId, wechateAccount, '7');
                this.$message(msg);
            } catch (error) {
                console.log(error);
            }
        ...
        // 生成城市列表数
        createCityInfo() {
            if (!this.cityInfo.currentCityId) {
                return;
            }
            const cityId = this.cityInfo.currentCityId.split(',');
            // 有可能是虚拟城市，所以需要展示虚拟城市name
            const virCityName = this.cityInfo.currentCityVirName.split(',');
            this.cityOptions = cityId.map((ele, idx) => {
                return {
                    value: ele,
                    label: virCityName[idx]
                };
            });
        },
        /**
    ...
    watch: {
            ...
            handler: function (val, oldval) {
                this.currentArticle =
                    this.articleList.length > 0 &&
                        this.articleList[this.currenChosen.classIndex].list[
                        this.currenChosen.subIndex
                        ]
                        ? this.articleList[this.currenChosen.classIndex].list[
                        this.currenChosen.subIndex
        ...
        show: {
            handler: function (val, oldval) {
                if (this.show) {
                    const cityId = this.cityId.split(',')[0];
                    this.getData(cityId);
                    this.createCityInfo();
                    // 初始化赋值
                    this.cityValue = cityId;
                    // 关闭右侧tab
...
.jl-slider-content {
    ...
    .jl-slider-r {
        ...
        /* 文章详情固定分享按钮 add by zhangchuang 始*/
        .jl-slider-footer {
            height: @height;
            line-height: @height;
            text-align: center;
        ...
        /* 文章详情固定分享按钮 add by zhangchuang 止*/
    }
}
.dialog-share {
    font-size: 14px;
